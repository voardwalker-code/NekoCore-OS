...
window._NVR = (() => {
  ...
  // Colors — matching arch doc REM-Architecture-v0.6.0.html palette
  const COLORS = {
    nodeDefault:  0x34d399,    // --accent emerald
    nodeActive:   0x22d3ee,    // --cyan
    nodeHot:      0xf87171,    // --danger red
    nodeSelected: 0xfbbf24,   // --gold
    edgeDefault:  0x1a4060,    // visible teal-blue edge
    edgeActive:   0x34d399,
    edgeTrace:    0x818cf8,    // --indigo
    particleColor:0x34d399,
    bgColor:      0x04060d,    // --bg0 / --surface-0
    gridColor:    0x080d18,    // --bg1 / --surface-1
    // Belief graph colors
  ...
  const MEMORY_TYPES = {
    core_memory: {
      ...
      glowScale: 6,
      label: 'Core',
      buildGeo: (s) => new THREE.SphereGeometry(s * 1.7, 32, 24)
    },
    episodic: {
      ...
      emissiveBase: 0.15,
      label: 'Episodic',
      buildGeo: (s) => new THREE.SphereGeometry(s, 28, 20)
    },
    semantic_knowledge: {
      ...
      emissiveBase: 0.2,
      label: 'Knowledge',
      buildGeo: (s) => new THREE.SphereGeometry(s * 1.05, 28, 20)
    },
    long_term_memory: {
      ...
      emissiveBase: 0.22,
      label: 'Long-Term',
      buildGeo: (s) => new THREE.SphereGeometry(s * 1.1, 28, 20)
    },
    semantic: {
      color: 0x60a5fa,       // blue — knowledge crystal
      emissive: 0x60a5fa,
    ...
    reflection: {
      ...
      emissiveBase: 0.2,
      label: 'Reflection',
      buildGeo: (s) => new THREE.SphereGeometry(s * 0.9, 28, 20)
    },
    interaction: {
    ...
    dream: {
      ...
      isDream: true,
      label: 'Dream',
      buildGeo: (s) => new THREE.SphereGeometry(s * 1.2, 32, 24)
    },
    dream_memory: {
      ...
      isDream: true,
      label: 'Dream Memory',
      buildGeo: (s) => new THREE.SphereGeometry(s * 1.15, 32, 24)
    },
    chatlog: {
      ...
      emissiveBase: 0.25,
      label: 'Chatlog',
      buildGeo: (s) => new THREE.SphereGeometry(s * 0.85, 28, 20)
    }
  };
  ...
  function init(containerEl, options = {}) {
    ...
    if (!document.getElementById('viz-tooltip-style')) {
      const tooltipStyle = document.createElement('style');
      tooltipStyle.id = 'viz-tooltip-style';
      tooltipStyle.textContent =
        '#viz-hover-tooltip{position:absolute;display:none;z-index:50;' +
        'background:rgba(8,13,24,0.93);backdrop-filter:blur(10px);' +
        'border:1px solid rgba(52,211,153,0.22);border-radius:8px;' +
        'padding:9px 13px;font-size:12px;font-family:"JetBrains Mono",monospace;' +
        'pointer-events:none;min-width:130px;max-width:210px;' +
        'line-height:1.85;box-shadow:0 4px 24px rgba(0,0,0,0.65);}';
    ...
    attribution.id = 'viz-attribution';
    attribution.style.cssText = 'position:absolute;bottom:8px;left:50%;transform:translateX(-50%);font-size:10px;color:rgba(100,130,160,0.38);pointer-events:none;font-family:"JetBrains Mono",monospace;z-index:10;letter-spacing:0.06em;white-space:nowrap;';
    attribution.textContent = 'three.js \u00b7 OrbitControls \u00b7 WebGL';
  ...
  function addNodeLabel(mesh, text, nodeSize, color) {
    ...
    sprite.scale.set(nodeSize * 5.5, nodeSize * 1.0, 1);
    sprite.position.set(nodeSize * 2.0, nodeSize * 0.5, 0);
    sprite.userData._label = true;
    mesh.add(sprite);
  }
  ...
  function showHoverTooltip(event, id) {
    ...
    if (!tooltip && container) {
      ...
      tooltip.id = 'viz-hover-tooltip';
      container.appendChild(tooltip);
    }
    if (!tooltip) return;

    const isBelief = beliefNodeMap.has(id);
    let html = '';
    if (isBelief) {
      const bd = beliefNodeData.get(id);
      html = '<div style="color:#c084fc;font-weight:700;margin-bottom:4px;">◇ Belief</div>';
      if (bd && bd.statement) {
        const stmt = bd.statement.length > 60 ? bd.statement.substring(0, 60) + '...' : bd.statement;
        html += '<div style="color:#a1a1aa;font-size:11px;">' + escapeHtml(stmt) + '</div>';
      }
    } else {
      const data = nodeData.get(id);
      if (!data) return;
      const typeKey = data.type || 'episodic';
      const profile = MEMORY_TYPES[typeKey] || DEFAULT_TYPE_PROFILE;
      const typeHex = '#' + profile.color.toString(16).padStart(6, '0');
      const icon = TYPE_ICONS[typeKey] || '●';
      html += '<div style="color:' + typeHex + ';font-weight:700;margin-bottom:5px;">' + icon + ' ' + escapeHtml(typeKey) + '</div>';
      const connIds = getConnectedNodeIds(id);
      const connTypes = new Map();
      for (const cid of connIds) {
        const cd = nodeData.get(cid) || beliefNodeData.get(cid);
        if (!cd) continue;
        const ct = beliefNodeData.has(cid) ? 'belief' : (cd.type || 'episodic');
        if (!connTypes.has(ct)) {
          const cp = ct === 'belief' ? { color: 0xc084fc, label: 'Belief' } : (MEMORY_TYPES[ct] || DEFAULT_TYPE_PROFILE);
          connTypes.set(ct, cp);
        }
      }
      for (const [ct, cp] of connTypes) {
        const hex = '#' + cp.color.toString(16).padStart(6, '0');
        const ic = TYPE_ICONS[ct] || '●';
        html += '<div style="color:' + hex + ';">' + ic + ' ' + escapeHtml(cp.label || ct) + '</div>';
      }
      if (connTypes.size === 0) {
        html += '<div style="color:#4a6a94;font-size:10px;">no connections</div>';
      }
    }
    ...
    tooltip.style.left = Math.min(x, container.clientWidth - 220) + 'px';
  ...
  function updateNeurochemOverlay() {
    ...
    overlay.innerHTML = chems.map(c => {
      ...
      return `<div style="display:flex;flex-direction:column;align-items:center;width:32px;">
        <div style="width:8px;height:40px;background:#0d1526;border-radius:4px;position:relative;overflow:hidden;">
  ...
  function revealNode(memId) {
    ...
    const mesh = nodeMap.get(memId);
    if (!mesh) return;
    if (_revealedNodeIds.has(memId)) { flashNode(memId, COLORS.nodeActive, 0.7); return; }
    ...
    mesh.material.color.setHex(COLORS.nodeActive);
  ...
  function destroy() {
    ...
    if (container) {
      const overlays = container.querySelectorAll('#viz-detail-panel, #viz-type-legend, #viz-neuro-overlay, #viz-hover-tooltip, #viz-attribution');
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .popout-btn { background: #0d1526; color: #e4e4e7; border: 1px solid rgba(45,122,237,0.3); border-radius: 6px;
    ...
    .viz-stat-bar { width:100%; height:4px; background:#0d1526; border-radius:2px; margin-top:2px; }
    .viz-stat-fill { height:100%; border-radius:2px; }
    .viz-detail-topics { display:flex; flex-wrap:wrap; gap:4px; margin-bottom:8px; }
    .viz-tag { background:#0d1526; color:#a1a1aa; padding:2px 6px; border-radius:4px; font-size:10px; }
...
<body>
  ...
  <script>
    ...
    loadScript('https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js', () => {
      loadScript('https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.js', () => {
        loadScript('${origin}/js/neural-viz/renderer.js', () => {
          loadScript('${origin}/js/neural-viz/data-layer.js', () => {
            loadScript('${origin}/js/neural-viz/index.js', () => {
  ...
  return {
    ...
    exitPlaybackMode,
    revealNode,
    // Extra exports for data-layer.js
    handleBrainEvent,
    flashNode,
    flashBeliefNode,
    debouncedBeliefReload,
    ...
    _setEventSource: (es) => { eventSource = es; },
