...
File: `server/services/voice-profile.js`
Function: `generateVoiceFromTraits(traits, gender)`

A voice profile is generated at entity creation and stored in `entity.json` under the `voice` key. It is loaded by the entity checkout route and delivered to the client in the checkout payload, where the humanizer uses it to control typing animation.

### Schema
```
{
  typingSpeed: {
    min   number    — minimum characters per second
    max   number    — maximum characters per second
  }
  rhythm: {
    punctuationPause   [minMs, maxMs]    — pause range after punctuation
    sentenceEndPause   [minMs, maxMs]    — pause range at sentence end
    newlinePause       [minMs, maxMs]    — pause range after newline
    burstChance        float             — probability of a fast-burst segment
  }
  errors: {
    typo             float    — rate of single character substitution
    transpose        float    — rate of adjacent character swap
    missedSpace      float    — rate of missing space between words
    doubleLetter     float    — rate of doubled letter
    wordCorrection   float    — rate of full word correction mid-type
    doubleCorrection float    — rate of correcting the correction (hesitation)
  }
  fillers: {
    chance   float      — probability of inserting a filler phrase
    phrases  string[]   — candidate filler strings
  }
  brb: {
    chance        float      — probability of a brb-style pause during response
    phrases       string[]   — phrases sent before the pause
    returnPhrase  string     — phrase sent when typing resumes
  }
}
```

### Generation rules
- Trait keywords map to modifier sets. Multiple traits stack additively.
- `gender` adjusts the default filler/brb phrase vocabulary.
- Unrecognized traits are silently ignored — the profile falls back to DEFAULTS.
- `getDefaultVoice()` returns the DEFAULTS object for entities without trait-driven customization.
