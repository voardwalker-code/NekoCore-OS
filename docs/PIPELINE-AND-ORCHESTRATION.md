...
- **Voice profile applied client-side**: the response text is humanized by the client using the entity's per-entity voice profile (`generateVoiceFromTraits`, `server/services/voice-profile.js`) — typing speed, rhythm, filler phrases, brb pauses, and error rates are all derived from the entity's personality traits at creation time.
...
### Memory Encoding
- `server/services/post-response-memory.js` calls **both** `createCoreMemory` and `createSemanticKnowledge` (dual-path IME — Phase 4.5)
