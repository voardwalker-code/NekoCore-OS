...
```json
{
  ...
  "voice": {
    ...
    "fillers": { "chance": 0.05, "phrases": ["..."] },
    "brb": { "chance": 0.035, "phrases": ["..."], "returnPhrase": "..." }
  }
}
```
...
Every entity receives a per-entity typing voice profile at creation. The profile is generated from the entity's personality traits and stored in `entity.json` under the `voice` key.

### Generation
`generateVoiceFromTraits` maps trait keywords (e.g. `precise`, `casual`, `bold`, `playful`, `shy`) to modifier sets that adjust the base DEFAULTS object. Multiple traits stack — the profile reflects the full personality blend.

### Voice schema
```
typingSpeed      { min, max }    — characters per second range
rhythm           { punctuationPause, sentenceEndPause, newlinePause, burstChance }
errors           { typo, transpose, missedSpace, doubleLetter, wordCorrection, doubleCorrection }
fillers          { chance, phrases }    — filler words randomly inserted
brb              { chance, phrases, returnPhrase }    — simulated away-keyboard pauses
```

### Usage
The client humanizer reads the entity's `voice` from the checkout payload and uses it to drive the typing animation. Each entity types at its own pace, makes its own characteristic errors, and uses its own filler/brb vocabulary — making entities distinguishable by their communication style as well as their words.
...
| server/services/voice-profile.js | Voice profile generation from personality traits |
